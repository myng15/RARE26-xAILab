...
# Set the output filename with timestamp and build information. Exported images are archived
# in container_images/ (excluded from the Docker build context via .dockerignore).
output_dir="${SCRIPT_DIR}/container_images"
mkdir -p "$output_dir"
output_filename="${output_dir}/${DOCKER_IMAGE_TAG}_${formatted_build_info}.tar.gz"
